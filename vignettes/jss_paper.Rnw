...
%\VignetteIndexEntry{Countr_guide}
...
\citep{chambers1990statistical} from package \pkg{stats} \citep{Rcore} or \code{hurdle()} and
...
to one:
% ***G: "shape" below was par2 (non-existent variable)
<<<>>=
par2Value <- 1.0
...
regressed on was the location parameter, or more precisely, the
