...
where $Y |x \sim Poisson (\E(Y|x) = \exp(x^\top \beta))$. Here
...
composite midpoint rule (\eg \citet[section 4.1.4]{press}):
\begin{equation}
...
extended-midpoint rule (\eg \citet{press}). The situation for convolutions is
...
\begin{table}[!htbp]
  ...
  \begin{tabular}{ll||llllll}
    ...
    $\mu, \sigma, q$\\
    burr-count & NSCF & burr    &   $\frac{\frac{kc}{\alpha} (\frac{t}{\alpha})^{c-1}}{\Big(1 + (\frac{t}{\alpha})^c     \Big)^{k+1}} $&
    $\alpha, c, k$\\

  \end{tabular}
  \caption{Built-in count distributions in package \pkg{Countr} and the interarrival time
    distributions generating them. The Poisson distribution is given for reference (it can be
    fitted using base \proglang{R}'s \code{glm()}). NSCF stands for \emph{no
...
\begin{equation}
     ...
     ,
\end{equation}
where $P_i = P_{y_i}(t|\mathbf{x}_i, \boldsymbol{\beta}_i)$ and $\frac{\partial
\mathcal{L}}{\partial \boldsymbol{\beta}}$ is a $q \times 1$ vector.
...
j$) and let $\hat{p}_j, j = 1, \dots, m$, be the fitted frequencies. For example, in
...
it is hard to conclude if $\hat{p}_j$ is a \textit{good approximation} to $\bar{p}_j$
...
respectively. All default to \code{TRUE}. If the variance-covariance matrix is
suppressed it is not computed at all.  The remaining arguments are specific to
...
and variance of \Sexpr{format(var(fertility$children), digits=4)}, %$
so there is a hint for under-dispersion in the response variable's marginal
distribution. A frequency table of this variable is shown in Table~\ref{tbl:freq}.
...
\subsubsection{Fitting built-in models}
...
\code{"..."} are the same argument that would be used in a call to
...
regressed on is the location parameter, or more precisely, the
...
the shape, the variance or other higher moments) via argument
\code{anc}. \code{anc} is a named list containing formulas describing regression
equations for the desired ancillary parameters.  Only the right-hand sides of
the formulas in argument \code{arg} are used. This setup is modelled on
(and is compatible with)
package \code{flexsurv} \citep[function \code{flexsurvreg()}]{flexsurv2016}.


In the ancillary regression case, starting values have to be provided.  We show
below an example of fitting such a model for the generalized gamma inter-arrival
times with covariates applied to all the distribution parameters.  The names of
the distribution parameters in this case are \code{"mu"} (location),
\code{"sigma"} and \code{"Q"} (see Table~\ref{tab:distparam}).  In this example
we will use the same formula, \code{regModel}, for the three parameters of the
distribution, so we prepare a list for the ancilliary parameters:
<<>>=
anc <- list(sigma = regModel, Q = regModel)
@

As discussed earlier, we obtain informative initial values for the location parameter from a
Poisson model. Here we can use the previously model \code{IV}.  We also set the intercepts
for (log) \code{"sigma"} and \code{"Q"} to one and the remaining regression coefficients to
zero. With the help of \code{renewalCoef()} all this can be done with a couple of lines:
<<>>=
...
\code{target}. In that case \code{renewalCoef()} assumes that the same formula is used for
all parameters.

%% This was before renewalCoef:
%%
%% <<eval=FALSE>>=
%% mu <- coef(IV)
...
\subsubsection{Custom inter-arrival distributions}
...
often a useful alternative.  The bootstrap computations in \pkg{Countr} are
...
weiWithBoot <- addBootSampleObject(weiModel, R = 400, parallel = "multicore",
  ncpus = 14)
\end{verbatim}
We use arguments \code{parallel} and \code{ncplus} to speed up the computations
...
% %% @georgi: this need to be changed and adapted to what we did in the fertility vignette
...
hence comparing information criteria is equivalent to comparing the
likelihoods. According to Table~\ref{tab:ic_models}, the gamma renewal model is
slightly preferred to the weibull model.
...
constructive suggestions, which helped us to improve the quality of the
software and the manuscript.


\bibliography{REFERENCES,football,quine,refresponse,var_covar}
